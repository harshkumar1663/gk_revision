...
def reflow_revisions(data, lecture_id, person=None):
    """Regenerate ALL revision dates for a lecture from scratch.
    
    Completely recalculates R1-R7 based on study_date, exam_date, difficulty,
    and interval_multiplier. No revision dates are preserved.
    
    GRADES ARE PRESERVED: Since grades are stored separately using lecture_id + "_" + stage,
    they remain intact even though revision dates move to new positions.
    
    Args:
        data: The full data dictionary
        lecture_id: The lecture to reflow
        person: Unused (kept for API compatibility)
    """
    ...
    # Completely regenerate all revision dates from scratch
    ...
    # Note: save_data() is NOT called here - caller is responsible for saving after all reflows
    ...
    with col1:
        ...
        if exam_date_input:
            ...
            if data["exam_date"] != new_exam_date:
                ...
                # Recalculate all lecture revision timelines
                for lecture_id in data["lectures"].keys():
                    reflow_revisions(data, lecture_id)

                # Globally rebalance schedules
                # for person in PERSONS:
